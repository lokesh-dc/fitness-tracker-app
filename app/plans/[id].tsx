...
export default function PlanDetailScreen() {
  ...
  async function loadData(isRefresh = false) {
    ...
    const [planResult, loggedResult] = await Promise.all([
      ...
      fetchLoggedDaysThisWeek(token!).then(days => ({ data: days, error: null })),
    ])
    ...
    if (planResult.data) {
      ...
      const todayDay = planResult.data.days.find(d => d.dayOfWeek === todayIndex)
      setSelectedDayOfWeek(todayDay ? todayIndex : planResult.data.days[0]?.dayOfWeek ?? null)
    } else {
  ...
  const selectedDay = plan?.days.find(d => d.dayOfWeek === selectedDayOfWeek) ?? null
